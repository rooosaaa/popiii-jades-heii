# Sown at Cosmic Dawn: Searching for Pop III Signatures in JWST JADES He II Selected Line Emitters

This repository contains supplementary data products associated with the paper:

> Sown at Cosmic Dawn: Searching for Pop III Signatures in JWST JADES He II Selected Line Emitters

The repository contains the broadband spectral energy distribution (SED)
overview plots for the galaxies analysed in the study.

## Contents

- `sed_overview/` — SED overview plots for individual galaxies.

The source IDs used in the filenames correspond to those adopted throughout
the paper.

## Data availability

A versioned release of these materials is archived on Zenodo:

[Zenodo DOI]

## Citation

If you use the data products in this repository, please cite the associated
paper and this dataset:

> Roberts, R. et al. (2026), "Sown at Cosmic Dawn: Searching for Pop III Signatures in JWST JADES He II Selected Line Emitters",
> Zenodo: https://doi.org/10.5281/zenodo.XXXXXXX

## License

The original figures and derived data products provided in this repository
are licensed under the Creative Commons Attribution 4.0 International
(CC BY 4.0) licence.

The underlying JWST observations and JADES data products are subject to
their respective data policies and are not relicensed by this repository.
